...
class WallFollowNode(object):
    ...
    def followHumanState(self):
        ...
        if self.updateDesiredAngle():
            ...
            # Run non-linear control based on distance
            # Slowly when far away, speed up when closer
            if self.obj_range > 2:
                twist_msg.linear.x = 0.3

            else:
                twist_msg.linear.x = (self.obj_range - 3) **8 + 0.3
                if twist_msg.linear.x > 5.0:
                    twist_msg.linear.x = 5.0

            # func_x = - (self.obj_range - 2) ** 5
            # if func_x < 0:
            #     func_x = 0
            # twist_msg.linear.x = func_x + 0.15
